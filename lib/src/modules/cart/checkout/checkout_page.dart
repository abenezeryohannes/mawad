...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  void initState() {
    ...
    _checkoutcontroller.calculatePayment(CaluatePayment(
      products: _cartController.cartItems
          .map((e) => ProductPayment(
                id: e.product.id,
                quantity: e.quantity,
              ))
          .toList(),
  ...
  Widget build(BuildContext context) {
    _addressController.getLocationDetail();
    return Obx(() {
      return _checkoutcontroller.invoice.value == null
                        ...
                        onClick: () {
                          if (_authController.userDetail.value!.name!.isEmpty ||
                            ...
                            _checkoutcontroller.makeOrder(OrderModel(
                                ...
                                percentageId:
                                    _checkoutcontroller.invoice.value!.id));
                          }
                        },
                      );
                    })
  ...
  Widget _buildPaymentListView() {
    return ListView.builder(
      ...
      shrinkWrap: true,
      itemCount: _checkoutcontroller.invoice.value!.items.length,
      itemBuilder: (context, index) {
        var paymentItem = _checkoutcontroller.invoice.value!.items[index];
        log('paymentItem==>: ${paymentItem.toJson()}');
        return Column(
          children: [
            SizedBox(
              height: 10.h,
            ...
            _buildTitle(
                '% ${paymentItem.percentage} : ${_getTitleForIndex(paymentItem.index)}'),
            SizedBox(
              height: 10.h,
            ),
            _buildPrimerCard(
